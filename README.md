...
- **Transparency on Confidence:** The confidence score is self-reported by the same model doing verification, not an independently-derived measure. To protect against model overconfidence, **any single failed criterion immediately forces a FAIL verdict regardless of overall confidence**.
